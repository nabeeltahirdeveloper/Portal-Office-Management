...
export default function EmployeePage() {
  ...
  const [roleFilter, setRoleFilter] = useState<string>("employee"); // default to employee
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch(`/api/users${roleFilter ? `?role=${roleFilter}` : ""}`);
  ...
  const openEditForm = (user: EmployeeUser) => {
    setIsEditing(true);
    setShowForm(true);
    setEditId(user.id);
    setFormData({
  ...
  const deleteUser = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEmployeeUsers((prev) => prev.filter((u) => u.id !== id));
      } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {["password", "confirmPassword"].map((name) => (
              <div key={name} className="flex flex-col gap-1 relative">
                ...
                <input
                  name={name}
                  type={showPassword[name as "password" | "confirmPassword"] ? "text" : "password"}
                  value={formData[name as keyof typeof formData]}
                  onChange={handleChange}
                  placeholder={`Enter ${name}`}
                  className="p-3 border rounded-lg focus:outline-none focus:ring-2 transition-all focus:ring-blue-500 bg-gray-50"
                ...
                <button
                  ...
                  onClick={() =>
                    setShowPassword((prev) => ({
                      ...prev,
                      [name as "password" | "confirmPassword"]: !prev[name as "password" | "confirmPassword"],
                    }))
                  }
                  className="absolute right-3 top-10 text-gray-600 hover:text-gray-800"
                >
                  {showPassword[name as "password" | "confirmPassword"] ? "🙈" : "👁️"}
                </button>
              </div>
            ))}
          </div>
