...
export default function HRPage() {
  ...
  const openEditForm = (user: HRUser) => {
    setIsEditing(true);
    setShowForm(true);
    setEditId(user.id);
    setFormData({
  ...
  const deleteUser = async (id: string) => {
    if (!confirm("Are you sure you want to delete this HR user?")) return;
    ...
    try {
      ...
      if (isEditing && editId) {
        res = await fetch(`/api/users/${editId}`, {
          ...
          body: JSON.stringify({ ...formData, role: "hr" }),
        });
      } else {
      ...
      {showForm && (
        ...
        >
          <h2 className="text-3xl font-semibold text-gray-800 pb-3 border-b">
            {isEditing ? "Edit HR" : "Create New HR"}
          </h2>
